...
theta = linspace(0,2*pi,221); %gera a superfície do condutor em 360 pontos

xcj = x(5); % posição x do centro condutor 1 fase a (para trocar o condutor avaliado deve-se mudar este valor)
ycj = y(5); % posição y do centro condutor 1 fase a (para trocar o condutor avaliado deve-se mudar este valor)
...
ylabel('Campo elétrico (V/cm)')
xlabel('Ângulo (°)')
%title('Campo elétrico superficial condutor cinco') %título (trocar ao mudar condutor avaliado)
grid
hold on
% yy5 = smooth(xp5,yp5,'rloess'); %comando suavizar a curva referência condutor 5
% plot(xp5,yy5,'--','LineWidth',2.5); %plot curva referência (trocar variáveis ao mudar condutor avaliado)
plot(xp5,yp5,'--','LineWidth',2.5); %plot curva referência (trocar variáveis ao mudar condutor avaliado)
xlim([0 360])
